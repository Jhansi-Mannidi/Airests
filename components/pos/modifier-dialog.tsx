  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="gap-0 overflow-hidden rounded-2xl p-0 shadow-elevated ring-0 max-sm:inset-x-0 max-sm:bottom-0 max-sm:top-auto max-sm:left-0 max-sm:max-h-[92dvh] max-sm:w-full max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-b-none sm:max-w-4xl"
      >
        <div className="grid max-h-[92dvh] grid-cols-1 md:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:grid-cols-[minmax(0,26rem)_minmax(0,1fr)]">
          <div className="relative h-40 overflow-hidden bg-muted sm:h-44 md:h-auto md:min-h-[32rem]">
            <Image
            ...
            <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
              <p className="text-[11px] font-medium uppercase tracking-[0.16em] text-white/80">{item.category}</p>
              <DialogHeader className="mt-1 gap-1">
                <DialogTitle className="text-left text-xl font-semibold leading-tight text-white drop-shadow-sm md:text-2xl">
                  {item.name}
                </DialogTitle>
                <DialogDescription className="line-clamp-2 text-left text-sm leading-relaxed text-white/90">
                  {item.description}
                </DialogDescription>
              </DialogHeader>
              <div className="mt-3 flex flex-wrap items-center gap-1.5">
                <span className="rounded-full bg-white px-2.5 py-1 font-mono text-sm font-semibold tabular-nums text-foreground">
                ...
                {item.dietary?.map((d) => (
                  <span
                    key={d}
                    className="inline-flex items-center gap-1 rounded-full bg-white px-2 py-1 text-[11px] font-medium text-foreground"
                  >
                ...
                {item.spice ? (
                  <span className="inline-flex items-center gap-0.5 rounded-full bg-white px-2 py-1 text-[11px] font-medium text-foreground">
                    {Array.from({ length: item.spice }).map((_, i) => (
                      <Flame key={i} className="size-3 fill-current text-warning" />
                    ))}
                  </span>
                ) : null}
              </div>
          ...
          <div className="flex min-h-0 flex-col bg-card">
                        ...
                        {group.options.map((opt) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                'flex min-h-12 w-full items-center gap-3 rounded-xl border px-3 py-3 text-left transition-all',
                                active
            ...
            <div className="flex items-center gap-2 border-t border-border bg-card px-3 py-3 sm:gap-3 sm:px-5 sm:py-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
              <div className="flex shrink-0 items-center rounded-xl border border-border bg-background p-0.5">
                ...
                <span className="w-7 text-center font-mono text-base font-semibold tabular-nums">{qty}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  className="size-10 rounded-lg"
              ...
              <Button
                size="lg"
                className="h-12 min-w-0 flex-1 rounded-xl px-3 text-sm font-semibold sm:h-11 sm:px-4"
                disabled={!canAdd}
                onClick={handleAdd}
              >
                <span className="truncate">Add to order</span>
                <span className="ml-2 shrink-0 font-mono tabular-nums">${(unitPrice * qty).toFixed(2)}</span>
              </Button>
            </div>
          </div>
        </div>
